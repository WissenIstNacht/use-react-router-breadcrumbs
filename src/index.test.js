...
const components = {
	...
	BreadcrumbNavLinkTest: ({ match }) => (
		<a role="link" to={match.pathname}>
			Link
		</a>
	),
...
describe("use-react-router-breadcrumbs", () => {
	...
	describe("When extending react-router config", () => {
		it("Should render expected breadcrumbs with sensible defaults", () => {
			const routes = [{ path: "/one", breadcrumb: "OneCustom" }, { path: "/one/two" }]
		...
		it("Should throw if non route is used in Routes object", () => {
			const DeclarativeRoutes = <div>div is not allowed as immediate child of Routes</div>
			expect(() => {
				createRoutesFromChildren(DeclarativeRoutes)
			}).toThrow()
		})
	...
	describe("Options", () => {
		describe("excludePaths", () => {
			...
			it("Should work with url params", () => {
				const routes = [{ path: "/a" }, { path: "/a/:b" }, { path: "/a/:b/:c" }]
				renderer({
					pathname: "/a/b/c",
					routes,
		...
		describe("defaultFormatter", () => {
			it("should be used if a breadcrumb is not provided for a specific path", () => {
				...
				renderer({
					...
					routes,
					options: {
						defaultFormatter: (breadcrumb) => breadcrumb.replace(/two/g, "changed"),
					},
				})
	...
	describe("Invalid route object", () => {
		...
		it("Should error if both `path` and `index` are provided", () => {
			...
			).toThrow("useBreadcrumbs: `path` and `index` cannot be provided at the same time")
		})
